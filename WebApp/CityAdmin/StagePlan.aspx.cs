...
public partial class ClassAdmin_StagePlan : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
           ...
           // grdStagePlan.DataBind();     
            ...
            {
                string popupScript = "alert('" + (string)GetLocalResourceObject("StageLegDeleted") + "');";
        ...
        try
        {
            ...
            if (grdStagePlan.DataKeys[lastRowIndex]["Status"].ToString().ToLower().Equals("not started") || grdStagePlan.DataKeys[lastRowIndex]["Status"].ToString().ToLower().Equals("noch nicht gestartet"))
            {
               ...
               _BindData();
                ...
                ce1.Value = (string)GetLocalResourceObject("StageLegDeleted");
            ...
            {
                string popupScript = "alert('" + (string)GetLocalResourceObject("LegAlreadyStarted") + "');";
                ClientScript.RegisterStartupScript(Page.GetType(), "script", popupScript, true);
                _BindData();
                grdStagePlan.DataBind();
    ...
    protected void btnAddRoute_Click(object sender, EventArgs e)
    {
        try
        {
           ...
           int result = objClassAdmin.SaveStagePlan(Convert.ToInt32(hdnStagePlanId.Value), Convert.ToInt32(hdnClassId.Value),
               ...
               // double.Parse(locationElement.Element("lat").ToString(), System.Globalization.CultureInfo.InvariantCulture);
                if (result > 0)
                {
                    _BindData();
                    string popupScript = "alert('" + (string)GetLocalResourceObject("StageLegAdded") + "');";
                    ClientScript.RegisterStartupScript(Page.GetType(), "script", popupScript, true);
                    //grdStagePlan.DataBind();
                }
                else
    ...
    protected void grdStagePlan_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                e.Row.Cells[3].Text = (totalDistanceCovered.ToString() + " Km / "
                    + totalDistance.ToString() + " Km").Replace(",", ".").ToString(); 
    ...
    protected void sdsStagePlan_Selecting(object sender, SqlDataSourceSelectingEventArgs e)
    {
