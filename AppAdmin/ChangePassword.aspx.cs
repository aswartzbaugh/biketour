        ...
        try
        {
            ...
            if (_dt != null && _dt.Rows.Count > 0)
            {
                if (txtCurrentPassword.Text.Trim() == _dt.Rows[0]["Password"].ToString())
                {
                    ...
                    {
                        lblmsg.Visible = true;
                        lblmsg.Text =(string)GetLocalResourceObject("MsgPswdChngFailed");//Password change failed.
                ...
                {
                    lblmsg.Visible = true;
                    lblmsg.Text = (string)GetLocalResourceObject("MsgWrongPswd"); //"You have not entered your current password correctly. Please try again";
